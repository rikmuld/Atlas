...
var DefaultPlanaSetup;
(function (DefaultPlanaSetup) {
    ...
    DefaultPlanaSetup.run = run;
})(DefaultPlanaSetup || (DefaultPlanaSetup = {}));
...
var SimpleTextureDrawing;
(function (SimpleTextureDrawing) {
    var cat; // you can see the type of grix you get when you use Grix.SOME_METHOD
    function setup() {
    ...
    SimpleTextureDrawing.run = run;
})(SimpleTextureDrawing || (SimpleTextureDrawing = {}));
//a clean version
var SimpleTextureDrawingClean;
(function (SimpleTextureDrawingClean) {
    var cat;
    function setup() {
...
})(SimpleTextureDrawingClean || (SimpleTextureDrawingClean = {}));
/*
 * ID : trans
 */
//everything about render transformations, moving, scaling, rotating etc..
var GrixTransformations;
(function (GrixTransformations) {
    var cat;
    var rotation = 0;
    function setup() {
        cat = Grix.fromTexture(Assets.loadImg("/scripts/tutorials/cat.png"));
    ...
    GrixTransformations.run = run;
})(GrixTransformations || (GrixTransformations = {}));
//clean version without comments
var GrixTransformationsClean;
(function (GrixTransformationsClean) {
    var cat;
    var rotation = 0;
    function setup() {
        cat = Grix.fromTexture(Assets.loadImg("/scripts/tutorials/cat.png"));
...
(function (PerspectiveAndInput) {
    ...
    function render(delta) {
        var view = Plena.getDefaultView();
        cat.setPivotMove(0.5, 0.5);
        if (Keyboard.isDown(Keyboard.KEY_S))
            cat.scaleWidthToSize(200); //0.2 of the height, only if we old s
        else
            cat.scaleWidthToSize(500); //half the height
    ...
    PerspectiveAndInput.run = run;
    function clockwise(event) {
        rotDir = 1;
    }
    function counterClockwise(event) {
        rotDir = -1;
    }
...
})(PerspectiveAndInput || (PerspectiveAndInput = {}));
//clean version
var PerspectiveAndInputClean;
(function (PerspectiveAndInputClean) {
    var cat;
    var rotation = 0;
    var rotDir = 1;
    var rotSpeed = 1;
...
})(PerspectiveAndInputClean || (PerspectiveAndInputClean = {}));
/*
 * ID : assets
 */
...
(function (MoreAssets) {
    ...
    var view;
    var rotate = 0;
    var random = (Math.random() * 12387); //making sure the spiral has different colors every time
    function setup() {
        var calibri = Assets.getFont(Font.CALIBRI, 48); //the number defines resolution, not size per so, since we can scale
        ...
        var hello = Assets.mkTextImg("Flight of the Bumblebee!", calibri); //also possible to set things such as max width (so you get new lines) and background color
        ...
        for (var i = 0; i < 5000; i++) {
            if (i % 3 == 0)
                crazy.quad(5, 5, (i % 50) * 10, Math.floor(i / 50) * 10);
        }
        crazy.populate();
        Plena.getDefaultView().fixedResolutionH(1000);
    }
    function render(delta) {
        ...
        if (music.currentTime() > 2) {
            for (var i = 0; i < 20; i++)
                drawSpiral(i); //increase the 20 in there to
        }
        else
            drawIntro();
    }
    function drawSpiral(offset) {
...
(function (MoreAssetsClean) {
    ...
    var view;
    var rotate = 0;
    var random = Math.random() * 12387;
    function setup() {
        var calibri = Assets.getFont(Font.CALIBRI, 48).fill(Color.Gray.BLACK);
        view = Plena.getDefaultView();
...
})(MoreAssetsClean || (MoreAssetsClean = {}));
/*
 * ID : sprite
 */
var Sprites;
(function (Sprites) {
    ...
    function setup() {
        ...
        var catSprite = Assets.loadSprite("/scripts/tutorials/cats.png", { safe: true });
...
function runExample(example) {
    switch (example) {
        case "setup":
            DefaultPlanaSetup.run();
            break;
        case "texture":
            SimpleTextureDrawing.run();
            break;
        case "trans":
            GrixTransformations.run();
            break;
        case "persp":
            PerspectiveAndInput.run();
            break;
        case "assets":
            MoreAssets.run();
            break;
